...
//       }
//     }
//     // Note: Don't set isLoading to false here as the redirect will handle the loading state
//   };
...
//         <div className="hidden lg:flex lg:w-1/3 bg-gradient-to-br from-indigo-500 to-purple-600 flex-col justify-center items-center text-white p-12">
//           <div className="text-center">
//             <Brain
//               size={100}
...
//               Your Second Brain
//             </h1>
//             <p className="text-xl text-indigo-100 leading-relaxed max-w-md">
...
//               />
//               <h1 className="text-3xl font-bold text-indigo-600">RMBL</h1>
//               <p className="text-gray-600 mt-2">Your Second Brain</p>
//             </div>
...
const AuthRMBL = () => {
  ...
  const handleGoogleAuth = async (): Promise<void> => {
    ...
    try {
      // Use signUp by default for OAuth - Clerk will automatically handle existing users
      ...
      // If the error indicates the user already exists, try sign in
      if (clerkError.errors?.[0]?.code === "form_identifier_exists") {
        try {
          ...
          const signInClerkError = signInErr as ClerkError;
          setError(
            signInClerkError.errors?.[0]?.message ||
  ...
  // Alternative: Simpler approach using sign-in first (most common pattern)
  const handleGoogleAuthSimple = async (): Promise<void> => {
    if (!isLoaded) return;
    ...
    try {
      // Try sign-in first (most users are returning users)
      if (signIn) {
        await signIn.authenticateWithRedirect({
          strategy: "oauth_google",
          redirectUrl: "/api/sso-callback",
      ...
      // If sign-in fails, user might be new - try sign-up
      try {
        if (signUp) {
          await signUp.authenticateWithRedirect({
            strategy: "oauth_google",
