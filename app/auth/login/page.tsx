...
const AuthRMBL = () => {
  ...
  // Alternative approach: Custom Google OAuth implementation
  const handleCustomGoogleAuth = async (): Promise<void> => {
    setIsLoading(true);
    setError("");

    try {
      // Option 1: Use window.location to redirect to your custom OAuth endpoint
      // This gives you more control over the OAuth consent screen
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="w-full lg:w-2/3 flex flex-col justify-center items-center p-8 lg:p-16">
          <div className="w-full max-w-md">
            ...
            <div className="space-y-6">
              {/* Use the original Clerk method - the app name issue is likely in Clerk dashboard settings */}
              <button
                onClick={handleGoogleAuth}
                disabled={isLoading}
                className={`w-full bg-white border-1 border-gray-200 text-gray-700 font-semibold py-4 px-6 rounded-xl flex items-center justify-center hover:bg-gray-50 hover:border-gray-300 transition duration-200 shadow-sm ${
